...
import { checkAuth, login, loginStart, loginSuccess } from '../../features/auth/authSlice';
...
export const LoginForm = ({ shadowActive }) => {
    ...
    return (
        <div className={styles.loginform} style={{boxShadow:shadowActive&&'0px -1px 20px 0px rgb(109 109 109)', margin:shadowActive&&'50px'}}>
            ...
            <h1 style={{ margin: 0 }}>Вход</h1>
                ...
                {emailIsValid&&<button disabled={(vereficationCode.length===8)} onClick={sendDataForLogin}>{loading?'Загрузка':'Войти'}</button>}
